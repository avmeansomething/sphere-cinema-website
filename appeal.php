...
<head>
    <meta charset="UTF-8">
    <title>Обращение в студию</title>
...
<body style="background: white">
    ...
    <div class="inner_info">
        <img src="images/appeal.jpg" alt="">
    </div>
    <div class="pages" style="margin-top: 1vw">
        <?php
        if (isset($_GET['page'])) {
            $page = $_GET['page'];
        } else $page = 1;
        ...
        $res = mysqli_query($link, "SELECT count(*) from comments");
        ...
        echo 'Страницы ';
        for ($i = 1; $i <= $str_pag; $i++) {
            echo "<a href=appeal.php?page=" . $i . ">" . $i . "</a>";
        }
        ?>
    </div>
        ...
        <div class="projects">
            <?php
            $result = mysqli_query($link, "SELECT round(avg(rating_assessment),1) as 'rate', films.film_id, film_preview, film_name, film_genre, film_country, 
            film_year, film_duration, comment_text, user_assessment, user_name
                FROM films left join rating on rating.film_id = films.film_id inner join comments on comments.film_id = films.film_id group by film_name LIMIT $art, $kol");
            ...
            <div class="project">
                ...
                <div class="other_inform" style="text-align: start;">
                    <div class="infor">
                        <p><b>Рейтинг фильма: ' . $rating . ' 
                        </b></p>
                    </div>
                    <p><b style="font-size:20px;">' . $row['user_name'] . ' пишет: </b><br> ' . $row["comment_text"] . '</p>
                    <p><b style="font-size:20px;">Oценка фильма: '.$row['user_assessment'].'</b></p>
                </div>
        </div>';
